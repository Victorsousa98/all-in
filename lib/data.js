...
// O Sheets devolve células formatadas como moeda já com o texto pronto
// ("R$ 1.234,56"), então não dá para converter direto com Number. Descarta
// tudo que não for dígito ou separador e resolve o formato pt-BR.
function parseNumber(value) {
  ...
  if (lastComma > lastDot) {
    ...
    // Só ponto. Grupos de 3 dígitos são separador de milhar pt-BR ("1.000",
    // "1.234.567"); qualquer outro tamanho é decimal ("30.5", "1,234.56").
...
// Marca nomes que correspondem a mais de um jogador. Nunca resolvemos um nome
// ambíguo escolhendo o primeiro: ele vira órfão explícito.
...
// Índice nome -> id, ou AMBIGUOUS quando dois jogadores compartilham o nome.
...
function resolveName(idByName, value) {
  const id = idByName.get(nameKey(value));
...
//
// No fallback, um nome que não resolve (inexistente ou ambíguo) é descartado do
// torneio: o jogador some daquela partida e o prêmio deixa de bater com a soma
// dos buy-ins registrados. É transitório — o backfill aborta nesses casos, e o
// caminho todo desaparece quando `--verify` reportar cobertura total.
function tournamentPlayerIds(tournament, idByName) {
  ...
  return {
    winnerId: resolveName(idByName, tournament.winner),
    participantIds: tournament.participants
      .map(name => resolveName(idByName, name))
      .filter(Boolean),
    migrated: false
  };
}
...
// Desfaz o vínculo conta Google <-> jogador. A sessão continua válida: o
// usuário volta ao onboarding sem precisar entrar de novo.
//
// Limpa e-mail, foto e a flag de administrador da linha. A flag é persistida
// por vínculo, então deixá-la para trás permitiria que a próxima conta a
// escolher esse jogador herdasse privilégio administrativo. Quem for admin de
// verdade recupera pelo ADMIN_EMAIL ao vincular novamente.
async function unlinkPlayer(session) {
  const context = await currentUserContext(session);
  if (!context.linked) return getAppData(session);

  const rows = await getPlayerRows();
  const target = rows.find(r => r.id === context.playerId);
  if (!target) throw new Error('Jogador não encontrado.');

  // Escreve apenas D:F. ID, nome e status ativo ficam intactos, e os torneios
  // referenciam o jogador por ID — nada do histórico é afetado.
...
async function completeOnboarding(session, payload = {}) {
  ...
  // Sair em silêncio devolvia 200 sem escrever nada, e o onboarding ficava
  // aberto sem explicação — indistinguível de um botão quebrado.
  if (existing.linked) {
    const err = new Error('Sua conta já está vinculada a um jogador.');
    err.statusCode = 409;
    throw err;
  }
...
async function renamePlayer(session, playerId, rawName) {
  ...
  // Em linhas migradas, D/E são snapshot histórico do nome na data da partida
  // e não são tocadas. Só as linhas sem K/L ainda dependem do nome para
  // resolver o jogador, então apenas essas são reescritas — por compatibilidade
  // até o backfill rodar.
  const tournaments = await getTournamentRows();
  const previous = nameKey(previousName);

  const affected = tournaments.filter(t =>
    !t.participantIds.length &&
    (nameKey(t.winner) === previous ||
      t.participants.some(p => nameKey(p) === previous))
  );

  for (const t of affected) {
    const winner = nameKey(t.winner) === previous ? name : t.winner;
    const participants = t.participants
      .map(p => (nameKey(p) === previous ? name : p))
      .join(' | ');
