...
async function getTournamentRows() {
  const rows = await getValues('Torneios!A2:J');
  return rows.map((r, index) => ({
    rowNumber: index + 2,
    id: String(r[0] || ''),
    date: String(r[1] || ''),
    ...
    winner: String(r[3] || ''),
    participants: String(r[4] || '').split('|').map(x => x.trim()).filter(Boolean),
    ...
    for (const name of t.participants) {
      if (!stats[name]) {
        stats[name] = {
          ...
          winRate: 0,
          roi: 0,
          photo: profiles[name.toLowerCase()]?.photo || ''
        };
      }
      ...
      if (name.toLowerCase() === t.winner.toLowerCase()) {
        s.wins += 1;
        s.received += t.prize;
      }
    }
...
async function renamePlayer(session, playerId, rawName) {
  ...
  // TODO: a aba Torneios ainda referencia jogadores por nome (colunas Vencedor
  // e Participantes). Deveria armazenar o player ID, tornando o rename uma
  // escrita única em Jogadores!B. Enquanto o schema não migra, reescrevemos as
  // células afetadas para preservar ranking e histórico.
  const tournaments = await getTournamentRows();
  ...
  // Escreve apenas a célula do nome: ID, ativo, e-mail, foto e admin ficam intactos.
...
async function addTournament(session, payload = {}) {
  ...
  const winner = normalizeName(payload.winner);
  const participants = Array.isArray(payload.participants)
    ? payload.participants.map(normalizeName).filter(Boolean)
    : [];
  ...
  const normalized = participants.map(x => x.toLowerCase());
  if (new Set(normalized).size !== participants.length) {
  ...
  if (!winner) throw new Error('Selecione o vencedor.');

  if (!participants.some(p => p.toLowerCase() === winner.toLowerCase())) {
    throw new Error('O vencedor precisa estar entre os participantes.');
  }

  const activeNames = new Set(
    (await getPlayers()).map(p => p.name.toLowerCase())
  );

  const invalid = participants.find(p => !activeNames.has(p.toLowerCase()));
  if (invalid) throw new Error(`Jogador inválido ou inativo: ${invalid}`);

  const prize = buyIn * participants.length;

  await appendValues('Torneios!A:J', [[
    crypto.randomUUID(),
    date,
    buyIn,
    winner,
